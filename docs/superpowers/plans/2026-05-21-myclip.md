...
## Task 1: Bootstrap Xcode project + dependencies

**Files:**
- Create: `myclip.xcodeproj` (via Xcode GUI)
- Create: `myclip/Info.plist` (auto-generated, then edited)
- Create: `README.md`
...
- Organization Identifier: `com.jekeun`
- Bundle Identifier (auto): `com.jekeun.myclip`
- Interface: **SwiftUI**
- Language: **Swift**
- Storage: **None**
- Include Tests: **on**

Save at `/Users/parkjaekeun/DEV/myclip/`. After creation, close Xcode.

- [ ] **Step 2: Verify project structure**

Run: `ls /Users/parkjaekeun/DEV/myclip/`
Expected: `myclip/  myclip.xcodeproj/  myclipTests/  docs/`

- [ ] **Step 3: Set `LSUIElement = YES` in Info.plist**

Open `myclip.xcodeproj` → select the `myclip` target → Info tab → add row:
- Key: `Application is agent (UIElement)`
- Type: Boolean
- Value: `YES`

This hides the Dock icon so we are menu-bar-only.

- [ ] **Step 4: Set deployment target to macOS 13**

Target → General → Minimum Deployments → macOS `13.0`.

- [ ] **Step 5: Add Swift Package dependencies**

File → Add Package Dependencies. Add both:

1. `https://github.com/groue/GRDB.swift` — exact rule, pick latest 6.x. Add product **GRDB** to target `myclip`.
2. `https://github.com/sindresorhus/KeyboardShortcuts` — exact rule, latest 2.x. Add product **KeyboardShortcuts** to target `myclip`.

- [ ] **Step 6: Build to confirm wiring**

Cmd-B in Xcode (or: `xcodebuild -project myclip.xcodeproj -scheme myclip -destination 'platform=macOS' build -quiet`).
...
```bash
git add -A
git commit -m "chore: scaffold Xcode project with GRDB and KeyboardShortcuts"
```

---

## Task 2: Replace SwiftUI App with AppKit-driven menu-bar shell

**Files:**
- Modify: `myclip/myclipApp.swift`
- Create: `myclip/App/AppDelegate.swift`
- Delete: `myclip/ContentView.swift` (auto-generated, unused)

- [ ] **Step 1: Move `myclipApp.swift` to `App/`**

In Xcode, drag the file into a new group `App`. Delete `ContentView.swift` (move to trash).

- [ ] **Step 2: Rewrite `myclipApp.swift`**
...
- [ ] **Step 1: Write `Pasteboard` protocol**
