...
## 🎉 앱스토어 라이브 → 직접배포 **마지막 배포 완료** (2026-08-26, 세션 12)
- [x] **App Store 출시 확인** — `HotSauce - System Monitor` 1.3.0 · 무료 · macOS 13+ · **2026-08-25 라이브**
- [x] **직접배포 1.3.0(빌드10) 공증 배포 = 직접배포 채널 마지막 버전.** 커밋 `f31dd39` → push → 프로덕션 검증 통과(appcast 1.3.0/빌드10 · DMG 200·3,593,782B · 페이지 CTA App Store · 릴리스노트 한/영)
- [x] **이전 안내 3중 배치** — ① 실행 시 1회 안내 창(도달 보장) ② 설정창 상설 안내 ③ Sparkle·웹 릴리스노트. 버튼은 `macappstore://` 로 App Store 앱 직행, 국가는 앱 언어로 kr/us
- [x] **웹 App Store 전환** — 핫소스 페이지 CTA 2곳 교체, 페이지 내 DMG 링크 0건, JSON-LD downloadUrl 도 스토어. (핸드오프의 "웹 스트림 과제"였던 다운로드 버튼 교체가 이걸로 해소)
- [x] 미결 2건 해소 — `dist/` 커밋 시작(피클과 파리티) · `images/` 48MB gitignore
- [x] `sparkle-appcast.sh` 에 릴리스노트 링크(`[텍스트](주소)`) 지원 추가
- **실측으로 확정**: 앱 파일은 App Store 설치 시 **자동 대체**된다(휴지통 흔적 없음) → "지우라" 대신 **"재실행하라"** 안내. 설정은 샌드박스라 안 넘어감(`~/Library/Preferences/` ↔ `~/Library/Containers/` 두 경로 실재 확인)
- [ ] **직접배포 appcast·DMG 는 당분간 유지** — 내리면 구버전 사용자의 Sparkle 확인이 404 로 실패해 마지막 안내조차 못 받는다. 충분히 이전된 뒤 정리
- [ ] 앱스토어 업데이트를 낼 일이 생기면 **빌드 11부터**(10은 직접배포에 소진)
- ⚠️ 지뢰 3개 기록: `NSAlert.runModal()` 은 이 앱에서 **버튼이 안 눌린다**(LSUIElement) · `NSWindow.center()` 는 창을 **화면 밖**에 놓는다 · `https://apps.apple.com` 은 **브라우저**를 연다. 상세는 `docs/HANDOFF.md` 세션 12

## 🍎 1.3.0 아이콘 규격화 + 배너 토글 + **앱스토어 심사 제출** (2026-08-15, 세션 11)
- [x] **앱 아이콘 macOS 규격화** — 1024 꽉 찬 iOS식 → 824 스퀘어클(연속곡률) + 사방 100px 여백 + 그림자. SwiftUI `ImageRenderer` 1회성 CLI 로 변환, 두 번들 icns 해시 일치 확인
- [x] **팝업 사이트 배너 ON/OFF 토글** — 설정 → 일반(기본 ON, `showSiteBanner`). 끄면 배너 블록 141.5유닛만 빠져 468×611.5 → 468×538pt. **넓힌 푸터 띠 120 유닛은 유지**(사용자 요청)
- [x] 팝업이 떠 있는 채로 토글해도 즉시 크기 반영 (`UserDefaults.didChangeNotification` 옵저버)
- [x] **1.3.0 / 빌드 9** 로 bump — 배너 토글이 새 기능이라 팀 규칙(정식 배포=minor↑) 적용
- [x] 빌드 9 아카이브·업로드 — 세션 10 지뢰 2개 재발 없음(서명 `Apple Development`, quarantine 0건) + Sparkle 0건 + 샌드박스 정상
- [x] **`docs/APPSTORE-SUBMISSION.md`** 작성 — 한/영 전체 메타데이터 복붙용 (글자 수 전부 검산)
- [x] **App Store 심사 제출 완료** — 무료 · 미국/대한민국. 앱 이름은 한국어 `HotSauce` / 영어(미국) `HotSauce - System Monitor`(하이픈 — 콜론은 중복으로 거부됨)
- [ ] 심사 결과 대기 → 리젝 시 사유 기록·대응
- [ ] 🎯 **앱스토어 출시 후 → 직접배포 1.3.0 = 마지막 배포 + 이전 안내** (사용자 확정 전략)
  - 웹은 아직 1.2.0(빌드7)이라 배너 토글·새 아이콘이 없다. `info.ts` 에 1.3.0 릴리스노트 신규 작성 필요(배너 토글 + **앱스토어 이전 안내**). **빌드 9는 소진 → 빌드 10부터**
  - ⚠️ 순서 주의: 앱스토어 출시 확정 **후**에 내야 이전 안내의 대상이 존재한다
- [ ] 🎯 **그 이후 직접배포 채널 마감** — 기존 사용자는 App Store 판으로 **자동 이전되지 않아** 수동 재설치 안내 필요 · appcast 는 남겨두는 편이 안전(내리면 구버전 Sparkle 확인 실패) · 웹 다운로드 버튼 → App Store 배지 교체(웹 스트림)
- [ ] 커밋 판단: `images/`(48MB 디자인 원본 → gitignore 권장) · `dist/`(52KB, 피클은 커밋 중 → 파리티 위해 커밋 권장)
