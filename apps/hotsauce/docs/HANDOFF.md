...
2. **🔴 직접배포 채널이 1.2.0(빌드7)에 멈춰 있다 — 1.3.0 공증 배포로 동기화할 것.**
   - 현재 웹 배포판에는 **배너 토글·새 아이콘이 없다.** 앱스토어(1.3.0)와 내용이 어긋난 상태.
   ...
   - ⚠️ **`web/src/i18n/info.ts` 에 핫소스 1.3.0 항목이 아직 없다.** 배너 토글 문구를 포함해 새로 써야 한다.
...
- **🚨 앱 이름은 현지화마다 개별로 전역 중복 검사를 받는다.**
  - `HotSauce : System Monitor` → 거부. **`HotSauce` 단독으로 줄여도 거부.** 앱 레코드 생성 때 확보한 이름(기본 언어=한국어)이 다른 현지화에서 자동으로 통하지 않는다.
  - 해결: **영어(미국) 이름을 한국어와 다르게** 주니 통과. (한국=HotSauce, 미국=다른 이름. 나라별로 다른 이름은 허용된다.)
  - ⚠️ **정확히 어떤 문자열로 확정했는지 이 문서에 기록되지 않았다 — ASC 에서 확인해 여기에 적어둘 것.**
