...
### ⚠️ 배포 상태에서 짚어둘 것 3가지 (다음 세션 필독)

1. **알려진 결함을 안고 배포했다.** 브라우저 모드의 배율 계산이 ~2배 틀려(`pxPerCss=0.977` vs 기하학적 ~1.95) 스텝당 드러난 내용의 절반이 유실될 수 있다. 베타 배지+안내 문구로 기대치를 맞춘 상태이며, **1.4.1 최우선 과제**. 해결책은 이미 확정적: Vision 캘리브레이션 대신 JS로 `window.devicePixelRatio`를 읽으면 그 값이 곧 `backingScale × 브라우저 줌` = 정확한 pxPerCss다. (아래 "1순위" 참고)
2. **진단 로그를 의도적으로 남겼다.** 계획서 §4.2b의 "배포 전 제거" 지침을 **베타 기간에 한해 보류**한 결정 — 사용자 실기 결함을 진단할 유일한 통로이기 때문. `~/Library/Logs/PICkle/scroll.log`(1MB 절단, 스크롤 좌표·픽셀 수치만 기록 — URL·페이지 텍스트 등 개인정보 없음, 리뷰에서 확인). **베타 딱지를 뗄 때 배지·안내 문구와 함께 제거할 것.**
3. **기존 사용자에게는 Sparkle이 하루 1회 확인으로 순차 배포**된다(즉시 전원 반영 아님). 기존 캡처(⌥⇧S/D/A) 경로는 코드가 안 바뀌었으므로(독립 리뷰가 바이트 단위 확인) 회귀 위험은 없다. 다만 **스크롤 캡처를 처음 쓰는 사용자에게는 화면기록 권한 프롬프트가 뜬다** — 일반 캡처만 쓰던 기존 사용자는 여전히 프롬프트를 안 본다.
