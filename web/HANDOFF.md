...
## 🔖 세션 이어받기 (2026-07-09, 인포 릴리스노트 '더보기' 통합 + 피클 안내 카드 제거 — **라이브 배포 완료**)

> ✅ **배포됨**: 커밋 `8b03255` → master push → Vercel. 라이브(`pizza-clip.com/info`) 반영 폴링 확인(통합 버튼 present · "새 소식" 카드 0). 변경: `web/src/components/pages/InfoPage.astro` + `web/src/i18n/info.ts` **2파일만**(핫소스 앱 스트림 WIP·`.claude/`는 커밋 제외).

- **더보기 버튼 통합**(`InfoPage.astro`): 피자·피클 컬럼마다 따로 있던 '더보기/접기' 버튼을 제거하고, **세 컬럼을 한꺼번에 접고 펴는 단일 버튼**을 `.info__grid` 바로 아래 중앙(`.releases-more--all` = `display:flex; width:fit-content; margin-inline:auto`)에 배치. 각 컬럼의 그라데이션 페이드(`.releases-fade`)는 컬럼별로 그대로 유지.
- **스크립트 재작성**: `setupCollapse()`가 `[data-collapse]` 중 **릴리스 4건 초과 컬럼만 모아**(`wraps` 배열), 단일 버튼 클릭 시 전부 동시에 `is-collapsed` 토글 + 각 컬럼 `--collapse-h` 측정(`getBoundingClientRect`). resize 시 전 컬럼 재측정. 셰브론 회전은 버튼이 컬럼 밖으로 나갔으므로 옛 `.releases-collapse:not(.is-collapsed) …` 대신 **`.releases-more[aria-expanded="true"] .releases-more__chev`**(180°) 기준으로 변경.
- **핫소스 컬럼 통일**: 기존엔 접힘 없이 고정이었는데 `data-collapse={>4?"":undefined}` + 조건부 페이드를 붙여 피자·피클과 동일 패턴으로. **현재 핫소스 4건이라 미접힘**(5건째부터 자동 접힘 + 통합 버튼에 편입).
- **피클 "새 소식이 쌓일 때마다…" 카드 삭제**: 점선 안내 카드(`.rel__more`)를 마크업·CSS·i18n에서 전부 제거. 그것만 쓰던 `relMoreRest`(ko/en) 값 + `Info` 인터페이스 항목 + `.rel__more*` CSS 블록까지 정리(`grep`으로 다른 사용처 없음 확인).
- **검증**: `npm run build` 11p 통과·콘솔 0. 프리뷰 한·영 모두 **버튼 1개**·클릭 시 피자(11)·피클(7) **동시** 펼침/접힘·라벨 더보기↔접기(EN Show more↔less)·셰브론 180°·그리드 중앙정렬(367px 일치). 라이브 배포 반영까지 폴링 확인.

### 다음에 할 일 (이월, 급하지 않음)
- ⚠️ **접속지역 자동 언어분기 실검증**(2026-06-14부터 미완) — VPN 해외IP→`/en/`, 한국→한국어. Edge 미들웨어라 배포 후에만 확인 가능.
- (정리) `consts.ts` 미사용 `PIZZA_HOME`·`NAV_LINKS`, 폐기 블로그 파일. (차후) 영문 전용 OG·튜토리얼 영상, 홈화면 아이콘(apple-touch) 아직 🍕.
- **앱 새 버전 나오면**: `info.ts` ko/en `releases`(피자)·`pickleReleases`·`hotsauceReleases` 맨 위에 추가 + `*Page.astro` softwareVersion + (피클/핫소스는) consts 다운로드 URL. **각 컬럼 4건 초과 시 자동으로 통합 '더보기'에 접힘.**
