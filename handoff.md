...
> 최종 업데이트: 2026-08-15 · **이 파일 = 모노레포 인덱스 + 공통 컨텍스트.** 개별 스트림 세부는 각 스트림 핸드오프에.
...
- 🌶 **핫소스 앱**: **🍎 App Store 심사 제출 완료**(1.3.0 빌드9 · 무료 · 미국/한국). 앱 아이콘 macOS 규격화 + 팝업 배너 ON/OFF 토글 포함. **🎯 전략 확정 — 앱스토어 출시 후 직접배포 1.3.0(마지막 배포 + 이전 안내)을 내고 직접배포 채널을 마감한다.** → `apps/hotsauce/docs/HANDOFF.md`
